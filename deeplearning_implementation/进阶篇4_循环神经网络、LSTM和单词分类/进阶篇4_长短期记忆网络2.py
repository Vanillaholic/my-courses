...
# 定义类LSTMModel，它继承了torch.nn中的Module模块
class LSTMModel(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        # 输出门对应的线性层
        self.i2h_out = nn.Linear(input_size + hidden_size, hidden_size)
        # 细胞状态对应的线性层
    ...
    def forward(self, input, hidden, cell):
        ...
        # 计算遗忘门输出
        ft = torch.sigmoid(self.i2h_forget(combined))
        # 计算输入门输出
        it = torch.sigmoid(self.i2h_input(combined))
        # 计算输出门输出
        ot = torch.sigmoid(self.i2h_out(combined))
        # 计算候选记忆细胞
        candidate = torch.tanh(self.i2h_cell(combined))
        # 根据遗忘门ft与上一时刻记忆细胞cell、输入门it
        # 与当前时刻的候选记忆细胞candidate
        # 一起计算出当前时刻的记忆细胞ct
        ct = ft * cell + it * candidate
        # 将ct输入至tanh，与输出门ot
        # 一起计算当前时刻的隐藏层输出hidden
        hidden = ot * torch.tanh(ct)
        # 使用线性层h2o，计算当前时刻的输出层输出output
        output = self.h2o(hidden)
        # 返回hidden、ct和output
        return hidden, ct, output

    # 为了初始化第一个隐藏层和记忆细胞的输出，
    # 设置函数init_hidden
    # 用于声明一个1×hidden_size大小空张量
...
if __name__ == '__main__':
    ...
    model = LSTMModel(n_letters, n_hidden, n_classes)  # 模型对象
    ...
    for epoch in range(10):  # 外层循环，代表了整个训练数据集的遍历次数
        ...
        for batch_idx, (data, label) in enumerate(train_load):
            hidden = model.init_hidden() #初始化一个空的隐藏层输出
            tensor = name_to_tensor(data[0]) #保存数据data[0]的张量
            letters_num = tensor.size()[0] #保存输入数据中包含的字符数量

            cell = model.init_hidden() #初始化一个空的记忆细胞输出
            # 前向传播:
            hidden, cell, output = model(tensor[0], hidden, cell)
            # 使用循环计算输入序列tensor的结果
            for i in range(1, letters_num):
                # 将每个时刻的字符tensor[i]
                # 与上一时刻的隐藏层输出hidden输入至模型model
                # 计算出这一时刻的隐藏层输出hidden、记忆细胞输出cell和输出层输出output
                hidden, cell, output = model(tensor[i], hidden, cell)
    ...
    # 将训练好的模型保存为文件，文件名为names.classify.lstm
    torch.save(model.state_dict(), 'names.classify.lstm')
